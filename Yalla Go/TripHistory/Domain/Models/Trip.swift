...
/// fields, so this model does not invent any. `fare` IS a real backend field
/// now (server-computed, frozen at request time), unlike the old
/// client-side-only estimate this replaced.
...
struct Trip: Identifiable, Equatable {
    ...
    /// Resolved client-side via reverse geocoding at request time, then
    /// frozen on the backend — never recalculated. `nil` for rides created
    /// before this feature existed, or when geocoding failed/was unavailable;
    /// display should fall back to the coordinate in that case.
    var pickupAddress: String? = nil
    var dropoffAddress: String? = nil
